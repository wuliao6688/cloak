...
func TestConfigValidation_HTTP3RacingRejectsUnsupportedNetworkOverrides(t *testing.T) {
	testCases := []struct {
		name   string
		option tls_client.HttpClientOption
	}{
		{name: "IPv4 restriction", option: tls_client.WithDisableIPV6()},
		{name: "IPv6 restriction", option: tls_client.WithDisableIPV4()},
		{name: "certificate pinning", option: tls_client.WithCertificatePinning(map[string][]string{"example.com": {"pin"}}, nil)},
		{name: "bandwidth tracker", option: tls_client.WithBandwidthTracker()},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := tls_client.NewHttpClient(nil,
				tls_client.WithClientProfile(profiles.Chrome_133),
				tls_client.WithProtocolRacing(),
				tc.option,
			)
			if err == nil {
				t.Fatal("expected incompatible HTTP/3 racing configuration to be rejected")
			}
		})
	}
}

func TestConfigValidation_HTTP3RacingRejectsProxy(t *testing.T) {
	_, err := tls_client.NewHttpClient(nil,
		tls_client.WithClientProfile(profiles.Chrome_133),
		tls_client.WithProtocolRacing(),
		tls_client.WithProxyUrl("http://proxy.example.com:8080"),
	)
	if err == nil || !strings.Contains(err.Error(), "TCP proxy") {
		t.Fatalf("expected HTTP/3 racing proxy rejection, got %v", err)
	}
}
