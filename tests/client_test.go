...
func newFingerprintTestClient(t *testing.T, options ...tls_client.HttpClientOption) tls_client.HttpClient {
	t.Helper()

	// tls.peet.ws currently serves an expired certificate. These tests verify
	// the observed TLS/HTTP fingerprint, not the endpoint's PKI configuration.
	testOptions := make([]tls_client.HttpClientOption, 0, len(options)+1)
	testOptions = append(testOptions, options...)
	testOptions = append(testOptions, tls_client.WithInsecureSkipVerify())

	client, err := tls_client.NewHttpClient(nil, testOptions...)
	if err != nil {
		t.Fatal(err)
	}
	return client
...
func chrome116WithPsk(t *testing.T) {
	...
	comparePSKOrInitialResponse(t, "chrome",
		clientFingerprints[chrome][tls.HelloChrome_112.Str()],
		clientFingerprints[chrome][tls.HelloChrome_112_PSK.Str()],
		resp,
	)
}
...
func chrome_146_PSK(t *testing.T) {
	...
	comparePSKOrInitialResponse(t, "chrome",
		clientFingerprints[chrome][profiles.Chrome_146.GetClientHelloStr()],
		clientFingerprints[chrome][profiles.Chrome_146_PSK.GetClientHelloStr()],
		resp,
	)
}
...
func chrome_150_PSK(t *testing.T) {
	...
	comparePSKOrInitialResponse(t, "chrome",
		clientFingerprints[chrome][profiles.Chrome_150.GetClientHelloStr()],
		clientFingerprints[chrome][profiles.Chrome_150_PSK.GetClientHelloStr()],
		resp,
	)
}
...
func compareResponse(t *testing.T, clientName string, expectedValues map[string]string, resp *http.Response) {
	t.Helper()
	compareTLSAPIResponse(t, clientName, expectedValues, decodeTLSAPIResponse(t, resp))
}

func comparePSKOrInitialResponse(t *testing.T, clientName string, initialValues, resumedValues map[string]string, resp *http.Response) {
	t.Helper()

	observed := decodeTLSAPIResponse(t, resp)
	expectedValues := resumedValues
	if observed.TLS.Ja3 == initialValues[ja3String] {
		// Public fingerprint services do not guarantee that they issue or accept
		// resumable sessions. The deterministic local resumption test covers the
		// PSK path when the remote service performs another full handshake.
		t.Logf("%s endpoint did not resume the TLS session; validating the initial fingerprint", clientName)
		expectedValues = initialValues
	} else if observed.TLS.Ja3 != resumedValues[ja3String] {
		t.Fatalf("unexpected TLS Ja3 for PSK test.\ninitial : %s\nresumed : %s\nactual  : %s\nclient: %s",
			initialValues[ja3String], resumedValues[ja3String], observed.TLS.Ja3, clientName)
	}

	compareTLSAPIResponse(t, clientName, expectedValues, observed)
}

func decodeTLSAPIResponse(t *testing.T, resp *http.Response) TlsApiResponse {
	t.Helper()
	...
	return tlsApiResponse
}

func compareTLSAPIResponse(t *testing.T, clientName string, expectedValues map[string]string, tlsApiResponse TlsApiResponse) {
	t.Helper()
