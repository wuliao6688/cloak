...
type socksContextDialer struct {
	...
	socksContextDialer proxy.ContextDialer
}

func newSocksContextDialer(socksDialer proxy.Dialer) socksContextDialer {
	contextDialer, _ := socksDialer.(proxy.ContextDialer)
	return socksContextDialer{socksDialer: socksDialer, socksContextDialer: contextDialer}
}

func (s *socksContextDialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	if s.socksContextDialer != nil {
		return s.socksContextDialer.DialContext(ctx, network, address)
	}
	...
	// Keep the result channel unbuffered so a connection cannot be queued after
	// the caller has already returned because its context was canceled. In that
	// case the goroutine below must observe ctx.Done and close the late result.
	result := make(chan dialResult)
	go func() {
		conn, err := s.socksDialer.Dial(network, address)
		select {
	...
	case dialResult := <-result:
		...
		return dialResult.conn, dialResult.err
	case <-ctx.Done():
		return nil, ctx.Err()
	}
}
...
func (c *connectDialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	case "https":
		if c.DialTLS != nil {
			...
			proxyConn, err := c.Dialer.DialContext(ctx, network, c.ProxyUrl.Host)
			if err != nil {
				return nil, err
			}
