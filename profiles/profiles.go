...
// canonicalTLSClients is the immutable registry used by resolvers. The
// exported MappedTLSClients map is retained for source compatibility, but
// external mutation must not change resolver behavior process-wide.
var canonicalTLSClients = maps.Clone(MappedTLSClients)
...
func NewClientProfile(clientHelloId tls.ClientHelloID, settings map[SettingID]uint32, settingsOrder []SettingID, pseudoHeaderOrder []string, connectionFlow uint32, priorities []Priority, headerPriority *PriorityParam, streamID uint32, allowHTTP bool, http3Settings map[uint64]uint64, http3SettingsOrder []uint64, http3PriorityParam uint32, http3PseudoHeaderOrder []string, http3SendGreaseFrames bool) ClientProfile {
	return ClientProfile{
		...
		settingsOrder:          slices.Clone(settingsOrder),
		pseudoHeaderOrder:      slices.Clone(pseudoHeaderOrder),
		connectionFlow:         connectionFlow,
		priorities:             slices.Clone(priorities),
		headerPriority:         clonePriorityParam(headerPriority),
		streamID:               streamID,
		allowHTTP:              allowHTTP,
		http3Settings:          maps.Clone(http3Settings),
		http3SettingsOrder:     slices.Clone(http3SettingsOrder),
		http3PriorityParam:     http3PriorityParam,
		http3PseudoHeaderOrder: slices.Clone(http3PseudoHeaderOrder),
...
func (c ClientProfile) GetHttp3Settings() map[uint64]uint64 {
	return maps.Clone(c.http3Settings)
}

func (c ClientProfile) GetHttp3SettingsOrder() []uint64 {
	return slices.Clone(c.http3SettingsOrder)
}
...
func (c ClientProfile) GetHttp3PseudoHeaderOrder() []string {
	return slices.Clone(c.http3PseudoHeaderOrder)
}
...
func cloneClientHelloID(id tls.ClientHelloID) tls.ClientHelloID {
	if id.Seed != nil {
		seed := *id.Seed
		id.Seed = &seed
	}
