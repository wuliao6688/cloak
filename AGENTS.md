...
3. **保留用户工作。**
   ...
   - 工作树不干净时，先让用户确认备份方式：创建 patch，或由用户自行提交。
   - 本项目的日常修改和上游同步都直接在本地 `master` 上进行，自动化工具不得创建额外工作分支或临时分支。
...
本项目明确要求直接在本地 `master` 上修改和同步，不创建 `codex/`、临时同步或其他工作分支：
...
如果存在本地非 `master` 分支，只有在工作树已备份且分支提交确实属于本项目维护内容时，才将其逐个合并到 `master`。不得把远程跟踪分支、WIP 分支或实验分支无差别合并。试验性修改使用仓库外 patch 备份和小步可审阅 diff，不通过新建分支隔离。
