// Package cloak provides a local HTTP/HTTPS proxy that applies TLS
// ClientHello fingerprints to outbound connections. Any HTTP client in
// any language can use it by setting HTTP_PROXY.
//
// # Architecture
//
//	HTTP  request: client → proxy (plain) → upstream (TLS via cloak)
//	HTTPS CONNECT: client → proxy (tunnel) → upstream (TLS via cloak)
//
...
// # Usage (Go library)
//
//	p := cloak.NewProxy(":8080", profiles.Chrome_150)
//	go p.ListenAndServe()
//
// # Usage (CLI)
//
//	go run ./cmd/cloak-proxy -addr :8080 -profile chrome_150
//
//	# Any language:
...
func (p *Proxy) ListenAndServe() error {
	...
	p.Logger.Printf("cloak proxy listening on %s (profile: %s)",
...
// handleHTTP forwards a plain HTTP proxy request through the cloak Transport.
