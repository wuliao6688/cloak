...
// quicConn abstracts both tls.QUICConn and tls.UQUICConn.
// UQUICConn is used when a uTLS ClientHelloID fingerprint is injected
// (HTTP/3 TLS fingerprinting); QUICConn otherwise.
// StoreSession is intentionally NOT part of the interface: utls's
// UQUICConn does not expose it (session caching is skipped for the
// fingerprint path, which is acceptable — resumption is optional).
...
func overrideALPN(spec *tls.ClientHelloSpec, alpn []string, tpData []byte) {
	...
	if !foundALPN {
		// No ALPN extension present — append one (shouldn't happen for
...
func NewCryptoSetupClient(
	...
	// Inject uTLS ClientHello fingerprint when requested.
	// UQUICClient (unlike QUICClient) builds the ClientHello from a
	// ClientHelloID, giving a browser-identical QUIC TLS fingerprint.
	// NOTE: utls's ClientHelloID.IsSet() is inverted (returns true for
	// EMPTY ids — an upstream quirk), so we test !IsSet() to mean "set".
	if !clientHelloID.IsSet() {
		// The profile's ClientHelloSpec advertises h2/http1.1 ALPN (from the
		// TCP fingerprint). QUIC requires ALPN "h3" and TLS 1.3 only, so we
		// override those while keeping every other fingerprint attribute.
		//
		// We must use HelloCustom + ApplyPreset: with a real ClientHelloID,
		// buildHandshakeState re-generates the spec from the ID on first use,
		// discarding any preset. HelloCustom tells utls "spec comes from
		// ApplyPreset only".
		...
		if specErr == nil {
			...
			if presetErr := uconn.ApplyPreset(&spec); presetErr == nil {
				cs.conn = uconn
			} else {
...
func (h *cryptoSetup) handleEvent(ev tls.QUICEvent) (err error) {
	...
	case tls.QUICStoreSession:
		...
		// UQUICConn (fingerprint path) doesn't implement StoreSession;
		// skip session caching in that case — resumption is optional.
		if ss, ok := h.conn.(interface{ StoreSession(*tls.SessionState) error }); ok {
			return ss.StoreSession(ev.SessionState)
		}
		return nil
