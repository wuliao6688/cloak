...
type Config struct {
	...
	// ClientHelloID, when set (client side only), injects the given uTLS
	// ClientHello fingerprint into the QUIC TLS 1.3 handshake.
	// This is the key to HTTP/3 TLS fingerprinting: without it the
	// ClientHello is Go's default (instantly detectable as non-browser).
	// Zero value (empty ClientHelloID) keeps quic-go's default behavior.
