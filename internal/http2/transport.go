...
type Transport struct {
	...
	// Settings is the initial SETTINGS frame to send to the peer.
	// If nil, a default set is used (standard Go HTTP/2 settings).
	// Set this to match a specific browser's SETTINGS fingerprint
	// (Chrome sends 5 settings in a specific order).
	Settings []Setting

	// InitialStreamID is the first stream ID the client uses.
	// Go default is 1; Chrome uses 3.
	InitialStreamID uint32

	// ConnectionFlow is the initial connection-level flow control window
	// advertised to the server. Chrome uses 15663105.
	ConnectionFlow uint32

	// PriorityFrames is an ordered list of PRIORITY frames to send
	// after the initial SETTINGS. Chrome sends 7 priority frames.
...
func (t *Transport) newClientConn(c net.Conn, singleUse bool, internalStateHook func()) (*ClientConn, error) {
	...
	// If custom Settings were provided, use those instead of defaults.
	if len(t.Settings) > 0 {
