// Command cloak-proxy runs a local HTTP forward proxy that applies
...
func main() {
	...
	log.Printf("cloak proxy starting with profile: %s", resolved.GetClientHelloStr())

	p := cloak.NewProxy(*addr, resolved)
	if *insecure {
