...
## 6. 追加对照实验(2026-08-26, 同一时刻同一站点)

用户质疑"能 push GitHub 说明网络没问题" — 对, 网络可达。真正拦截来自
**服务端指纹检测**, 用同环境同站点连续 5 次对照验证:

### 多 WAF 对照(同一环境)

| 站点 | curl(默认指纹) | cloak(Chrome_150) |
|---|---|---|
| Cloudflare | 200 | 200 |
| **Akamai** | **403** | **200** |
| Imperva | 200 | 200 |
| F5 | 200 | 200 |
| **hCaptcha** | **302**(重定向检测页) | **200** |
| Sannysoft | 检测页 | **PASS** |

**分水岭**: Cloudflare/Imperva/F5 基础防护(UA 检查即可过), 但
**Akamai 与 hCaptcha 会检测 TLS/行为指纹** — curl 被拒, cloak 通过。

### Akamai 官网(www.akamai.com)

| 客户端 | 第1次 | 第2次 | 第3次 | 第4次 | 第5次 |
|---|---|---|---|---|---|
| 🔴 curl(默认TLS指纹) | 403 | 403 | 403 | 403 | 403 |
| 🟢 cloak(Chrome_150指纹) | 200 | 200 | 200 | 200 | 200 |

**唯一变量是 TLS 指纹** — 网络环境、站点、时间完全相同:
- curl 的 Go/curl 默认指纹 → Akamai 判定机器人 → **403 拒绝**
- cloak 的 Chrome 150 指纹 → Akamai 判定浏览器 → **200 放行**
- 连续 5 次稳定, 非抖动, 非网络问题

### 结论修正

网络层可达(push GitHub 证明), 检测发生在**应用层(WAF 指纹识别)**:
- "能不能连上" = 网络问题(代理冷启动/DNS 抖动, 已用 warm-up 规避)
- "连上后被不被放行" = **指纹有效性的真正考验** → cloak 通过

指纹有效性由**服务端判定结果**证明, 而非连通性。
