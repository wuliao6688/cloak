...
5 分钟上手 cloak。核心入口就三个：`Impersonate`（标准）、`ImpersonateH3`（QUIC）、
...
resp, err := cloak.ImpersonateRequest(profiles.Chrome_150).
	...
	SetRetry(3, cloak.RetryOnServerError,      // 重试 3 次
		1*time.Second, 10*time.Second).          // 退避 1s-10s
	SetDump(cloak.DefaultDumpOptions()).       // 打印请求/响应
...
```go
// JSON body
resp, _ := cloak.ImpersonateRequest(profiles.Chrome_150).
...
```go
info := cloak.ImpersonateChain(profiles.Chrome_150).
